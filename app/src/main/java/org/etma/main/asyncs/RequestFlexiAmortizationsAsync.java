...
public class RequestFlexiAmortizationsAsync extends AsyncTask<String, Void, String> {

    private Context context;

    public RequestFlexiAmortizationsAsync(Context contxt){
    ...
    protected void onPostExecute(String response) {
        ...
        if ( response != null ){
            if (Integer.parseInt(response) != 0){
                EventBus.getDefault().post( new FlexiAmortizationReceivedEvent(true, Integer.parseInt(response)));
            }else{
                EventBus.getDefault().post( new FlexiAmortizationReceivedEvent( true, 0));
            }
        }else{
            EventBus.getDefault().post( new FlexiAmortizationReceivedEvent( false, 0 ));
        }
