...
public class SendPasswordResetCodeAsync extends AsyncTask<String, Void, String> {

    @SuppressLint("StaticFieldLeak")
    public  Context context;

    public SendPasswordResetCodeAsync(Context context){
    ...
    protected void onPostExecute(final String result) {
        ...
        if (result != null){
            PasswordResetCodeResponse resetCodeResponse = gson.fromJson(result, PasswordResetCodeResponse.class);

            if (resetCodeResponse.isSuccess()){
                EventBus.getDefault().post( new PasswordResetEmailSentEvent(true));
            }else{
                EventBus.getDefault().post( new PasswordResetEmailSentEvent(false));
            }
        }else{
            EventBus.getDefault().post( new PasswordResetEmailSentEvent(false));
        }
